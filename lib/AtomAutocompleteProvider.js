...
export default class AtomAutocompleteProvider {
    ...
    async getSuggestions({editor, bufferPosition, prefix: origPrefix}) {
        ...
        } else if (nline.indexOf("layout:") >= 0) {
            rightLabel = 'Blog Layout';
            list = this.data.file.layouts;
        } else if (nline.indexOf("category:") >= 0) {
            rightLabel = 'Blog Category';
            list = this.data.file.categories;
        }
